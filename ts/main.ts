...
// --- Gallery panel --------------------------------------------------------
//
// Lazy-loads ts/public/gallery.json (a static, hand-pinned catalogue
// generated by `go run -tags=kolavatardev ./cmd/kolavatar-gallery-json`)
// and renders all 100 entries as small SVGs in a wall. Clicking any tile
// loads its descriptor into the main canvas, reusing the same render path
// as story selection.

const galleryWall = $('#gallery-wall');
const gallerySym = $<HTMLSelectElement>('#gallery-filter-sym');
const galleryStatus = $('#gallery-status');
const galleryPanel = $<HTMLDetailsElement>('#gallery-panel');
...
async function loadGallery(): Promise<void> {
  ...
  try {
    ...
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    galleryEntries = (await res.json()) as GalleryEntry[];
    galleryStatus.textContent = `${galleryEntries.length} designs`;
    galleryStatus.className = 'status ok';
    renderGalleryWall();
  } catch (err) {
    galleryStatus.textContent = `failed: ${(err as Error).message}`;
    galleryStatus.className = 'status error';
  }
}

function renderGalleryWall(): void {
  if (!galleryEntries) return;
  const filterSym = gallerySym.value;
  const filtered = filterSym
    ? galleryEntries.filter((e) => e.symmetry === filterSym)
    : galleryEntries;
  const out: string[] = [];
  for (const e of filtered) {
    ...
    out.push(
      `<button class="gallery-tile" data-id="${escapeHtml(e.id)}" ` +
        `title="${escapeHtml(e.id)}">` +
        `<div class="gallery-thumb">${svg}</div>` +
        `<div class="gallery-tile-label">${escapeHtml(e.title)}</div>` +
        `</button>`,
    );
  }
...
galleryWall.addEventListener('click', (ev) => {
  const target = (ev.target as HTMLElement).closest<HTMLElement>('[data-id]');
  if (!target || !galleryEntries) return;
  const id = target.dataset.id;
  const entry = galleryEntries.find((x) => x.id === id);
  if (!entry) return;
  state.descriptor = structuredClone(entry.descriptor);
  state.storyId = '';
  setActiveLink('');
  titleEl.textContent = `Gallery — ${entry.title}`;
  blurbEl.textContent =
    `Symmetry ${entry.symmetry}, grid ${entry.grid}, score ${entry.score.toFixed(2)}, ` +
    `seed "${entry.seed}". Click any other tile to compare.`;
  jsonTextarea.value = JSON.stringify(state.descriptor, null, 2);
  setJsonStatus('', '');
  void loadGoReference(undefined);
  rerender();
});

gallerySym.addEventListener('change', renderGalleryWall);

// Lazy-load on first open so unfolding doesn't pay the 100-render cost
// when the user only wants the live API panel.
galleryPanel.addEventListener('toggle', () => {
  if (galleryPanel.open) void loadGallery();
});
